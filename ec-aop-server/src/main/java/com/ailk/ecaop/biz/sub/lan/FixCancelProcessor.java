...
@EcRocTag("fixcancel")
public class FixCancelProcessor extends BaseAopProcessor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        msg.put("soNbr", msg.get("oldProvOrderId"));
        body.put("msg", msg);
        exchange.getIn().setBody(body);
        // 撤单许可预判

        // lan.preData("ecaop.trade.scmc.isAllowCancelSoParametersMapping", exchange);
        // CallEngine.wsCall(exchange, "ecaop.comm.conf.url.cbss.services.orderChgandCleSer");
        // lan.xml2Json("ecaop.trade.scmc.isAllowCancelSoTemplate", exchange);
        // // lan.("ecaop.trade.scmc.isAllowCancelSoTemplate", exchange);
        //
        // Map outMap = exchange.getOut().getBody(Map.class);
        // if (null != outMap.get("code") && !"0".equals(outMap.get("code"))) {
        // throw new EcAopServerBizException("9999", null == outMap.get("detail") ? outMap.get("detail").toString()
        // : "撤单预判失败");
        // }
        ...
        CallEngine.wsCall(exchange3, "ecaop.comm.conf.url.cbss.services.ordForNorthSer");
        ...
        // 正式提交
        lan.preData("ecaop.trades.scmc.cancel.paramtersmapping", exchange4cBOrderSub);
    ...
    private Map preTradeOtherData(Map msg) {
        ...
        item.put("rsrvStr3", null == msg.get("cancelReason") ? msg.get("cancelReason").toString() : "撤单");
    ...
    private Map preTradeItemData(Map msg) {
        ...
        tempMap4.put("attrValue", null == msg.get("cancelReason") ? msg.get("cancelReason").toString() : "撤单");
    ...
    private Map preTradeItem(Map msg, String appCode) {
        ...
        // IS_ORDER_TRACK_CP-融合订单撤单，IS_ORDER_TRACK_UU-关联单撤单，IS_ORDER_TRACK-普通订单撤单，单产品撤单属于普通订单撤单 modify by wangrj3
        item.add(LanUtils.createTradeItem("IS_ORDER_TRACK", msg.get("oldProvOrderId").toString()));// 塞原订单流水
        item.add(LanUtils.createTradeItem("STANDARD_KIND_CODE", ""));// 值不知道怎么生成，先塞空吧
        item.add(LanUtils.createTradeItem("CANCELTRACK_REASON",
                null == msg.get("cancelReason") ? msg.get("cancelReason").toString() : "撤单"));// 塞撤单原因
        tradeItem.put("item", item);
        return tradeItem;
    }
    ...
    private Map preTradeOther(Map msg, String tradeId, Exchange exchange) {
        ...
        tempMap.put("rsrvValue", msg.get("oldProvOrderId").toString());// 待撤单原订单流水号
        tempMap.put("rsrvStr1", "track");
        tempMap.put("rsrvStr2", null == msg.get("cancelReason") ? msg.get("cancelReason").toString() : "撤单");// 撤单原因
        ...
        tempMap.put("rsrvStr9", msg.get("oldProvOrderId").toString());// 最原始的TRADE_ID,传入用于撤单通知
        ...
        tempMap.put("rsrvStr21", msg.get("oldProvOrderId").toString());
