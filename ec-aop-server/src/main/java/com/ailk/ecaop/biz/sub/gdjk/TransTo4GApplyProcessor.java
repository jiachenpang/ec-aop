...
/**
 * 23转4开户申请--适用于智慧沃家融合业务
 * 
 * @author GaoLei
 */
@EcRocTag("TransTo4GApply")
@SuppressWarnings(value = { "unchecked", "rawtypes" })
public class TransTo4GApplyProcessor extends BaseAopProcessor {
    ...
    public TransTo4GApplyProcessor(ParametersMappingProcessor[] pmp) {
        this.pmp = pmp;
    }
    ...
    public Map applyChangeTo4G(Exchange exchange) throws Exception {
        ...
        // copy一个Exchange
        Exchange c23GExchange = ExchangeUtils.ofCopy(exchange, msg);
        Map checkResult = check23GTransfer(c23GExchange, true);
        Map oldCust = ((List<Map>) checkResult.get("custInfo")).get(0);
        Map oldUser = ((List<Map>) checkResult.get("userInfo")).get(0);
        Map oldAcct = ((List<Map>) checkResult.get("acctInfo")).get(0);
        ...
        String userCityCode = (String) (StringUtils.isEmpty((String) oldUser.get("cityCode")) ? cityCode : oldUser.get(
                "cityCode"));
        ...
        if ("0".equals(msg.get("mainCardTag")) && "3".equals(msg.get("addType"))) {
            ...
            if (!IsEmptyUtils.isEmpty(subItemList)) {
                subItemList.add(MapUtils.asMap("xDatatype", "NULL", "attrTypeCode", "0", "attrCode", "MAIN_CARD_TAG",
                        "attrValue", "0", "itemId", msg.get("userId"), "startDate",
                        msg.get("nextMon1stDay"), "endDate", MagicNumber.CBSS_DEFAULT_EXPIRE_TIME));
            }
        ...
        // 增加行销标示
        System.out.println("移网迁转行销标示" + msg.get("markingTag"));
