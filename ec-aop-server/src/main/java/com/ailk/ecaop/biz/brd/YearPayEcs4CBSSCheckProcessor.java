...
import com.ailk.ecaop.biz.sub.lan.YearPayEcs4CBSSCheck;
...
@EcRocTag("yearPayEcs4CBSSCheck")
public class YearPayEcs4CBSSCheckProcessor extends BaseAopProcessor implements ParamsAppliable {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        ELKAopLogger.logStr("请求进来的msg: appTx: " + apptx + ", msg= " + msg);
        ...
        ELKAopLogger.logStr("处理三户后的msg: appTx: " + apptx + ", msg= " + msg);
        //通过relationTypeCode判断是不是融合号码（88或者89开头的）
        System.out.println("appTx: " + apptx + ", msg= " + msg.get("relationTypeCode"));
        if (!IsEmptyUtils.isEmpty(msg.get("relationTypeCode"))) {
            ...
            if ("1".equals(msg.get("changeTag")) || IsEmptyUtils.isEmpty(msg.get("changeTag"))) {
                ...
                // 成员用户预提交
                Map phoneMap = prePhoneNumberInfo(exchange, msg, threePartInfo, flag, mixMap);
                //正式提交，销毁订单
            ...
            YearPayEcs4CBSSCheck check = new YearPayEcs4CBSSCheck();
    ...
    private Map callThreePart(Exchange exchange, Map msg) throws Exception {
        ...
        if (!IsEmptyUtils.isEmpty(uuInfo)) {
            ELKAopLogger.logStr("处理UUINFO: appTx: " + apptx + ", 走进来了么 ");
            for (Map temMap : uuInfo) {
                System.out.println("APPTX: " + apptx + ", uuinfo= " + temMap);
        ...
        for (Map discnt : discntInfo) {
            ...
            msg.put("discntName", prodTariffDescribe);
    ...
    private Map prePhoneNumberInfo(Exchange exchange, Map msg, Map threePartInfo, Boolean flag, Map mixMap) {
        Exchange copyExchange = ExchangeUtils.ofCopy(exchange, msg);
        Map ext = new HashMap();
        ELKAopLogger.logStr("处理成员前的msg: appTx: " + apptx + ", msg= " + msg);
        if (flag) {
            // 宽带所需的资费和生效方式
        ...
        try {
            ...
            cancelMixOreder(mixMap, exchange, msg);
            e.printStackTrace();
            throw new EcAopServerBizException(MagicNumber.ONS_ERROR_CODE, "调用成员预提交失败.");
        }
    ...
    private void cancelMixOreder(Map mixMap, Exchange exchange, Map msg) {
        ...
        dealMixSubmit(submitMap, mixMap, msg);
    ...
    private Map dealMixSubmit(Map submitMap, Map mixMap, Map msg) {
    ...
    private Map preDataForTradeItem(Map msg, Boolean isCY, Boolean flag) {
        ...
        if (flag) {
            if (isCY) {
                ...
                Item.add(lan.createTradeSubItemC(discntitemId, "aDiscntCode", msg.get("delayDiscntId"), startDate,
                        endDate));
                Item.add(lan.createTradeSubItemC(discntitemId, "bDiscntCode", "", startDate,
                        endDate));
    ...
    private void orderChgSub(Map mixMap, Map phoneMap, Exchange exchange, Map msg) throws Exception {
        ...
        if (!IsEmptyUtils.isEmpty((List<Map>) msg.get("payInfo"))) {
            ...
            for (Map payInfo : payInfoList) {
                pay.put("payType", ChangeCodeUtils.changePayType4N6odsb(payInfo.get("payType")));
                pay.put("payMoney", submitMap.get("origTotalFee"));
                pay.put("subProvinceOrderId", msg.get("provOrderId2"));
            }
            payList.add(pay);
            submitMap.put("payInfo", payList);
        }
        ...
        outMap.put("productName", msg.get("productName"));
        outMap.put("discntName", msg.get("discntName"));
        outMap.put("startDate", msg.get("startDate4Order"));
        String endDate = DateUtils.lastDay((String) msg.get("startDate4Order"));
        outMap.put(
                "endDate",
                DateUtils.addMonths(DateUtils.getBeforeDayLastTime(endDate),
                        (String) msg.get("orderEndDate")));
        Message out = new DefaultMessage();
        out.setBody(outMap);
        exchange.setOut(out);
    }
