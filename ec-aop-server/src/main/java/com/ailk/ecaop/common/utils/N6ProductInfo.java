...
public class N6ProductInfo implements ProductInfoUtil {
        ...
        if (productInfo != null && productInfo.size() > 0) {
            for (int i = 0; i < productInfo.size(); i++) {
                ...
                if (MAIN_PRODUCT.equals(productMode)) {
                    ...
                    // 剔除特定资费
                    delSpecialDis(msg, productInfoResult);
                    // // 检查是否有需要特殊处理的元素,并进行处理
                    // Map specialMap = new HashMap();
                    // specialMap.put("dealFlag", "REPLACE"); // 操作类型
                    // specialMap.put("productFlag", "N6"); // 老产品流程
                    // specialMap.put("productId", productId);
                    // specialMap.put("provinceCode", provinceCode);
                    // specialMap.put("eparchyCode", eparchyCode);
                    // specialMap.put("elementTypeCode", "D"); // 元素类型
                    // ProductUtils.elementSpecialDeal(specialMap, productInfoResult);
                    // System.out.println("productInfoResultDeal:" + productInfoResult);
                    ...
                    for (int j = 0; j < productInfoResult.size(); j++) {
                        if ("D".equals(productInfoResult.get(j).get("ELEMENT_TYPE_CODE"))) {
                            ...
                            // 处理主资费的开始时间和结束时间by wangmc 20170223
                            ...
                            if (packageElementInfo != null && packageElementInfo.size() > 0) {
                                for (int a = 0; a < packageElementInfo.size(); a++) {
                                    ...
                                    if ("D".equals(packageElementInfo.get(a).get("ELEMENT_TYPE_CODE"))) {
                                        ...
                                        // 处理主资费的开始时间和结束时间 by wangmc 20170223
            ...
            for (int i = 0; i < activityInfo.size(); i++) {
                ...
                for (int j = 0; j < actInfoResult.size(); j++) {
                    ...
                    if (packageElement != null && packageElement.size() > 0) {
                        ...
                        boolean isTure = "mabc".equals(appCode) && packageElement != null && packageElement.size() > 0;
    ...
    /**
     * 主资费的结束时间依据主产品的开始结束时间位基础,再按照表中配置偏移,by wnagmc 20170223 FIXME
     * @param dis 资费节点Map
     * @param discntCode 资费ID
     * @return true-该元素已失效,不下发;false-该元素未失效,进行下发
     */
    private boolean dealMainProDiscntDate(Map dis, Object discntCode) {
        Map discntDateMap = TradeManagerUtils.getMainProDiscntEffectTime(discntCode + "", GetDateUtils.getDate(), "");
     ...
     */
    private void delSpecialDis(Map msg, List<Map> productInfoResult) {
        Map para = new HashMap();
        para.put("SUBSYS_CODE", "CSM");
        para.put("PARAM_ATTR", msg.get("province"));// 2位省份编码
        para.put("PARAM_CODE", "delSpecialDiscnt");
        para.put("PARA_CODE1", "ZZZZ");
        para.put("EPARCHY_CODE", msg.get("eparchyCode"));
        para.put("PROVINCE_CODE", "00" + msg.get("province"));
        para.put("PARA_CODE3", msg.get("appCode"));
        List<Map> discntInfo = dao.qryDelSpecialDis(para);
        System.out.println("####山东测试2####" + discntInfo);
        if (null == discntInfo || discntInfo.isEmpty()) {
            return;
        }
        String discntList = (String) discntInfo.get(0).get("PARA_CODE2");
        List<Map> newProductionInfo = new ArrayList<Map>();
        for (int m = 0; m < productInfoResult.size(); m++) {
            System.out.println("####山东测试1####" + productInfoResult.get(m));
            if (discntList.contains(productInfoResult.get(m).get("ELEMENT_ID").toString())) {
                newProductionInfo.add(productInfoResult.get(m));
            }
        }
        productInfoResult.removeAll(newProductionInfo);
    }
