 ...
 */
@EcRocTag("rioNewuOpenApp4N6")
public class RioNewuOpenApp4N6BssProcessors extends BaseAopProcessor implements ParamsAppliable {
    ...
    private void preExtData(Exchange exchange, Map msg) {
        ...
        ext.put("tradePurchase", prePurchaseData(msg));
        // ext.put("tradeElement", preTradeElementData(msg));
        msg.put("ext", ext);
    }

    private Map prePurchaseData(Map msg) {
        try {
            Map tradePurchase = new HashMap();
            Map item = new HashMap();
            item.put("userId", msg.get("userId"));
            item.put("bindsaleAttr", "88888888");
            item.put("deviceName", msg.get("attachName"));
            item.put("deviceType", msg.get("terminalType"));
            item.put("imei", msg.get("imei"));
            List<Map> simCardData = (List<Map>) msg.get("simCardData");
            if (null != simCardData && simCardData.size() > 0) {
                for (Map simCardDataa : simCardData) {
                    item.put("deviceBrand", simCardDataa.get("eid"));
                }
            }
    ...
    private Map preProductData(Map msg) {
        try {
            ...
            for (int i = 0; i < product.getN6ProductInfo().size(); i++) {
                Map item = new HashMap();
                item.put("xDatatype", "NULL");
                item.put("userId", msg.get("userId"));
                item.put("productMode", product.getN6ProductInfo().get(i)[3]);
                item.put("productId", product.getN6ProductInfo().get(i)[0]);
                item.put("brandCode", product.getN6ProductInfo().get(i)[2]);
    ...
    private Map preTradeSvcData(Map msg) {
        try {
            ...
            for (int i = 0; i < product.getN6ServiceInfo().size(); i++) {
                for (int j = 2; j < product.getN6ServiceInfo().get(i).length; j++) {
                    Map item = new HashMap();
                    item.put("xDatatype", "NULL");
                    item.put("userId", msg.get("userId"));
                    item.put("modifyTag", "0");
                    item.put("startDate", msg.get("date"));
                    item.put("endDate", MagicNumber.CBSS_DEFAULT_EXPIRE_TIME);
                    item.put("productId", product.getN6ServiceInfo().get(i)[0]);
                    item.put("packageId", product.getN6ServiceInfo().get(i)[1]);
                    item.put("serviceId", product.getN6ServiceInfo().get(i)[j]);
    ...
    private Map preDiscntData(Map msg) {
        try {
            ...
            List<Map> itemList = new ArrayList<Map>();
            for (int i = 0; i < product.getN6DiscntInfo().size(); i++) {
                for (int j = 2; j < product.getN6DiscntInfo().get(i).length; j++) {
                    Map item = new HashMap();
                    item.put("xDatatype", "NULL");
                    item.put("id", msg.get("userId"));
                    item.put("idType", "1");
                    item.put("userIdA", "-1");
                    item.put("productId", product.getN6DiscntInfo().get(i)[0]);
                    item.put("packageId", product.getN6DiscntInfo().get(i)[1]);
                    item.put("discntCode", product.getN6DiscntInfo().get(i)[j]);
                    item.put("specTag", "1");
    ...
    private Map preProductTpyeListData(Map msg) {
        try {
            ...
            for (int i = 0; i < product.getN6ProductInfo().size(); i++) {
                ...
                item.put("productMode", product.getN6ProductInfo().get(i)[3]);
                item.put("productId", product.getN6ProductInfo().get(i)[0]);
                item.put("productTypeCode", product.getN6ProductInfo().get(i)[1]);
                item.put("modifyTag", "0");
                item.put("startDate", msg.get("date"));
    ...
    private Map preDataForTradeUser(Map msg) {
        try {
            ...
            item.put("brandCode", product.getN6ProductInfo().get(0)[2]);
            item.put("productId", product.getN6ProductInfo().get(0)[0]);
            item.put("eparchyCode", msg.get("eparchyCode"));
            item.put("cityCode", msg.get("district"));
            // 活动类型
            item.put("productTypeCode", product.getN6ProductInfo().get(0)[1]);
            item.put("userPasswd",
                    msg.get("serialNumB").toString().substring(msg.get("serialNumB").toString().length() - 6));
            item.put("userTypeCode", "T");
    ...
    private Map preDataForTradeSubItem(Map msg) {
        try {
            ...
            // 北六需要
            Item.add(lan.createTradeSubItemE("MAIN_SN_CODE", msg.get("serialNumA"), (String) msg.get("userId")));
    ...
    private Map preDataForTradeRelation(Map msg) {
        try {
            ...
            Map item = new HashMap();
            item.put("xDatatype", "NULL");
            item.put("idA", msg.get("userIdMain"));
            ...
            item.put("xDatatype", "NULL");
            item.put("relationTypeCode", "ONSC");
            item.put("modifyTag", "0");
            item.put("startDate", GetDateUtils.getDate());
            item.put("endDate", MagicNumber.CBSS_DEFAULT_EXPIRE_TIME);
            item.put("remark", "一号多卡");
            itemList.add(item);
    ...
    private void threepartCheck(Exchange exchange, Map msg) {
        ...
        msg.put("userId",
                GetSeqUtil.getSeqFromN6ess(pmp[1], ExchangeUtils.ofCopy(exchange, msg), "USER_ID",
                        (String) msg.get("eparchyCode")));
        msg.put("idA", "-1");
        String serialNumber = (String) msg.get("serialNumA");
        String orderid = GetSeqUtil.getSeqFromN6ess(pmp[1], ExchangeUtils.ofCopy(exchange, msg), "TRADE_ID",
                (String) msg.get("eparchyCode"));
        // 查询cycId
        ...
        String payRelId = GetSeqUtil.getSeqFromN6ess(pmp[1], ExchangeUtils.ofCopy(exchange, msg), "TRADE_ID",
                (String) msg.get("eparchyCode"));
        ...
        msg.put("userIdMain", checkNull(userInfo.get(0), "userId"));
    ...
    private void preBaseData(Exchange exchange, Map msg) {
        try {
            List<String[]> productInfo = product.getN6ProductInfo();
            ...
            base.put("productId", productInfo.get(0)[0]);
            base.put("tradeStaffId", msg.get("operatorId"));
            base.put("userDiffCode", "00");
            base.put("brandCode", productInfo.get(0)[2]);
    ...
    private void dealReturn(Exchange exchange, Map msg) {
        ...
        msg.put("mProductId", product.getN6ProductInfo().get(0)[0]);
        msg.put("mBrandCode", "4G00");
