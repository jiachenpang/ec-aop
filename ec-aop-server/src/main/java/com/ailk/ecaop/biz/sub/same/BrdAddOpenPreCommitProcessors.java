...
@EcRocTag("brdAddOpenPreCommit")
public class BrdAddOpenPreCommitProcessors extends BaseAopProcessor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        lan.preData("ecaop.masb.baoa.sUniTradeParametersMapping", exchange);
    ...
    private Map dealParam(Exchange exchange) throws Exception {
        ...
        if (iptvInfos != null) {
            for (Map map : iptvInfos) {
                msg.put("elementId", map.get("IptvDiscnt"));
                discntList.add(TradeManagerUtils.preProductInfoByElementId(msg));
            }
        }
        if (interTvInfos != null) {
            for (Map map : interTvInfos) {
                msg.put("elementId", map.get("interTvDiscnt"));
                discntList.add(TradeManagerUtils.preProductInfoByElementId(msg));
            }
        }
        // 获取产品信息
        ...
        inMap.put("CB_ACCESS_TYPE", newUserInfo.get("accessMode"));
        ...
        try {
            if (resultSet != null && resultSet.size() > 0) {
                cbAccessType = (String) resultSet.get(0);
            }
            else {
        ...
        item3.put("payrelationId", GetSeqUtil.getSeqFromCb(ExchangeUtils.ofCopy(exchange, msg), "seq_trade_id"));
        item3.put("actTag", "1");
        ...
        subitems.add(lan.createTradeSubItem("USETYPE",
                MixBroadBandAndLandLineToolUtils.changeUserPropertyCode(userProperty), itemId));
        ...
        subitems.add(lan.createTradeSubItem("ACCESS_TYPE", newUserInfo.get("accessMode"), itemId));
        ...
        subitems.add(lan.createTradeSubItem("ISWAIL",
                MixBroadBandAndLandLineToolUtils.changeAcceptModeCode(acceptMode), itemId));
        ...
        subitems.add(lan.createTradeSubItem("TERMINAL_TYPE", machineProvide, userId));
        subitems.add(lan.createTradeSubItem("TERMINALSRC_MODE",
                MixBroadBandAndLandLineToolUtils.changeTerminalCode(machineType), userId));// ?
        ...
        subitems.add(lan.createTradeSubItem("TOWN_FLAG", MixBroadBandAndLandLineToolUtils.changeCityMarkCode(cityMark),
                itemId));
        ...
        subitems.add(lan.createTradeSubItem("TERMINAL_TYPE", "1", itemId));
        ...
        subitems.add(lan.createTradeSubItemE("LINK_NAME", msg.get("contactPerson"), userId));
        subitems.add(lan.createTradeSubItemE("OTHERCONTACT", "", itemId));
        subitems.add(lan.createTradeSubItemE("LINK_PHONE", msg.get("contactPhone"), userId));
