...
public class YearForHB {
        ...
        for (Map rspMap : phoneRspInfo) {
            ...
            if (null != provinceOrder && provinceOrder.size() > 0) {
                for (Map orderInfo : provinceOrder) {
                    ...
                    provOrderId = orderInfo.get("subProvinceOrderId");
                }
            }
        }
    ...
    private Map prePhoneNumberInfo(Exchange exchange, Map msg, Map threePartInfo, Boolean flag)
            throws Exception {
        Map ext = new HashMap();
        ...
        int cycle = 12;
        String itemId = "";
        String itemId1 = (String) GetSeqUtil.getSeqFromCb(pmp[6], ExchangeUtils.ofCopy(exchange, msg),
                "seq_item_id", 1).get(0);
        msg.put("discntItemId", itemId1);
        for (Map para : paraList) {
            if ("PROD_TARIFF_CODE".equals(String.valueOf(para.get("paraId")))) {
                prodTariffCode = para.get("paraValue");
            }
            if ("PROD_TARIFF_DESCRIBE".equals(String.valueOf(para.get("paraId")))) {
        ...
        for (Map user : userInfo) {
            ...
            for (Map discnt : discntInfo) {
                ...
                if (!IsEmptyUtils.isEmpty(attrInfo)) {
                    for (Map attr : attrInfo) {
                        if ("cycle".equals(attr.get("attrCode"))) {
                            cycle = Integer.valueOf((String) attr.get("attrValue"));
                        }
                ...
                String yyEndDate = sdf.format(RDate.addMonths(sdf.parse(startDate), cycle));
                /**
                ...
                if (month > 15) {
                    ...
                    yyEndDate = GetDateUtils.getMonthLastDayFormat1(yyEndDate);
                }
                else {
                    yyEndDate = DateUtils
                            .theMonthLastTime(DateUtils.addMonths(DateUtils.addSeconds(yyEndDate, 1), "-1"));
                }*/
                String yyStartDate = "";
                if (month > 15) {
                    yyStartDate = GetDateUtils.getNextDiscntDate(yyEndDate);
                }
                else {
                    yyStartDate = yyEndDate.substring(0, 8) + "000000";
                }
                yyEndDate = DateUtils.addSeconds(yyStartDate, -1);
                yyEndDate = GetDateUtils.getMonthLastDayFormat1(yyEndDate);
                String endDate = sdf.format(RDate.addMonths(sdf.parse(yyStartDate), cycle));
    ...
    /*
    private void dealReturn(Exchange exchange, Map mixMap, Map phoneMap, Map msg) {
     List fee = new ArrayList();
     List<Map> rspInfo = (List) mixMap.get("rspInfo");
     List<Map> phoneRspInfo = (ArrayList<Map>) phoneMap.get("rspInfo");
     if (null == phoneRspInfo || phoneRspInfo.isEmpty()) {
         throw new EcAopServerBizException("9999", "N6ESS统一预提交未返回应答信息");
     }
     Map realOut = new HashMap();
     Integer totalFee = 0;
     Object provOrderId = "";
     realOut.put("provOrderId2", rspInfo.get(0).get("bssOrderId"));
     for (Map rspMap : phoneRspInfo) {
         List<Map> provinceOrder = (List) rspMap.get("provinceOrderInfo");
         if (null != provinceOrder && provinceOrder.size() > 0) {
             for (Map orderInfo : provinceOrder) {
     ...
     List<Map> feeinfo = dealFee(phoneRspInfo);

     realOut.put("code", "0000");
     realOut.put("detail", "OK");
     realOut.put("provOrderId", provOrderId);
     realOut.put("arrearageMess", msg.get("arrearageMess"));
     realOut.put("custName", msg.get("custName"));
     realOut.put("productType", "1");
     realOut.put("productName", msg.get("productName"));
     realOut.put("discntName", msg.get("discntName"));
     realOut.put("startDate", msg.get("yyStartDate"));
     realOut.put("endDate", msg.get("yyEndDate"));
     realOut.put("feeInfo", feeinfo);
    ...
    private Map preDataForTradeSubItem(Map msg, Boolean flag) {
        ...
        if (flag) {
            ...
            String productId = (String) msg.get("newProductId");
            String speed = changeSpeed(productId);
            String attrCode = changeAttrCode(speed);
            Item.add(lan.createTradeSubItem(attrCode, speed, (String) msg.get("itemId")));
        }
        else {
            Item.add(lan.createTradeSubItemC((String) msg.get("itemId"), "fixedHire", msg.get("fixedHire"),
                    startDate, endDate));
            Item.add(lan.createTradeSubItemC((String) msg.get("itemId"), "expireDealMode", "a", startDate, endDate));
            Item.add(lan.createTradeSubItemC((String) msg.get("itemId"), "bDiscntCode",
                    (String) msg.get("bDiscntCode"),
                    startDate,
                    endDate));
    ...
    private void preTradeSubItemNotSD(List<Map> item, Map msg, String disItemId, LanUtils lan, String brandNumber,
        ...
        if ("a".equals(expireDealMode)) {
            item.add(lan.createTradeSubItemC(disItemId, "bDiscntCode", msg.get("bDiscntCode"),
                    chgStartDate, endDate));
        }
        ...
        item.add(lan.createTradeSubItemC(disItemId, "bcafixedhireflag", "0", chgStartDate,
                endDate));
        ...
        item.add(lan.createTradeSubItemC(disItemId, "effectMode", delayType, chgStartDate,
                endDate));
        ...
        if (!IsEmptyUtils.isEmpty(msg.get("totalFee"))) {
            item.add(lan.createTradeSubItemC(disItemId, "cycleFee", msg.get("totalFee"), chgStartDate, endDate));
        }
        else {
        ...
        item.add(lan.createTradeSubItemC(disItemId, "fixedHire", brandNumber, chgStartDate,
                endDate));
    ...
    public static String changeAttrCode(Object speed) {
        Map changeMap = MapUtils.asMap("98044", "C_98", "98045", "C_98", "98046", "C_98", "98034", "C_98",
                "802987", "C_26", "802986", "C_26", "802978", "C_26", "802990", "C_26", "802897", "C_26");
