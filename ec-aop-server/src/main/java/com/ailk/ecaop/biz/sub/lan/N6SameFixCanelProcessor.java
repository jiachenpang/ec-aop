 ...
 * 北六固网同装撤单
 */
@EcRocTag("N6samefixcanncel")
public class N6SameFixCanelProcessor extends BaseAopProcessor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        // 预提交
        lan.preData("ecaop.trades.mofc.N6.cancelPre.paramtersmapping", exchange);
    ...
    private void orderSubPreData(List<Map> rspInfo, Map preDateMap) {
        ...
        if (null != feeInfoList && feeInfoList.size() > 0) {
            ...
            for (Map feeMap : feeInfoList) {
                ...
                dealRequestFee(feeMap);
                Map subOrderMap = MapUtils.asMap("subProvinceOrderId", provOrderId, "subOrderId", orderNo, "feeInfo",
                        feeList);
                subOrderSubReqList.add(subOrderMap);
            }
        }
        ...
        preDateMap.put("origTotalFee", preDateMap.get("origTotalFee"));
