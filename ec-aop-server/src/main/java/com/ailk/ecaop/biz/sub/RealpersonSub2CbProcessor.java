 ...
 * @author liuzg
 */
@EcRocTag("RealpersonSub2Cb")
public class RealpersonSub2CbProcessor extends BaseAopProcessor implements ParamsAppliable {
    ...
    private Map callPreSubmit(Exchange exchange, Map msg, Map threePartOut) {
        ...
        String tradeId = (String) GetSeqUtil.getSeqFromCb(pmp[3], ExchangeUtils.ofCopy(exchange, msg), "seq_trade_id",
                1).get(0);
        String itemId = (String) GetSeqUtil.getSeqFromCb(pmp[3], ExchangeUtils.ofCopy(exchange, msg), "seq_item_id", 1)
                .get(0);
        ...
        Map preDateMap = MapUtils.asMap("tradeId", tradeId, "itemId", itemId, "newCustId", custInfo.get("custId"),
                "productId", userInfo.get("productId"), "brandCode", userInfo.get("brandCode"), "custId",
                custInfo.get("custId"), "userId", userInfo.get("userId"), "acctId", acctInfo.get("acctId"));
    ...
    private Map preTradeSubItem(Map inMap, Map msg) {
        ...
        if ("07|13|14|15|17|18|21|20|23".contains(certTypeCode)) {
            items.add(lan.createTradeSubItem("REAL_PERSON_TAG", msg.get("realPersonTag"), (String) inMap.get("itemId")));
        }
        else {
            items.add(lan.createTradeSubItem("REAL_PERSON_TAG", msg.get("realPersonTag"), (String) inMap.get("itemId")));
