 ...
 */
@EcRocTag("oldUserRenActivityN6")
public class OldUserRenActivityN6 extends BaseAopProcessor {
    ...
    private void preDataForPreCommit(Exchange exchange) {
        try {
            ...
            String orderid = GetSeqUtil.getSeqFromN6ess(ExchangeUtils.ofCopy(exchange, msg), "TRADE_ID", eparchyCode);
            ...
            String ItemId = GetSeqUtil.getSeqFromN6ess(ExchangeUtils.ofCopy(exchange, msg), "ITEM_ID", eparchyCode);
            ...
            body.put("msg", msg);
            System.out.println("=============================打印预提交msg：" + msg);
    ...
    private Map preDataForTradeSubItem(Map msg) {
        try {
            ...
            //
            // }
            System.out.println(";';discntItemid:" + msg.get("discntItemid"));
            ...
            // itemList.add(lan.createTradeSubItemC(msg.get("itemId").toString(), "tradeId", msg.get("tradeId"),
    ...
    private Map preTradeItem(Map msg) {
        ...
        itemList.add(LanUtils.createTradeItem("IPHONE4PRODUCTID", (String) msg.get("mainProduct")));
        ...
        itemList.add(LanUtils.createTradeItem("DEVELOP_DEPART_ID", ""));
        itemList.add(LanUtils.createTradeItem("MARKETING_MODE", "1"));
        itemList.add(LanUtils.createTradeItem("IMMEDIACY_INFO", "0"));
        itemList.add(LanUtils.createTradeItem("DEVELOP_STAFF_ID", ""));
        // itemList.add(LanUtils.createTradeItem("USERID_BAK", msg.get("userId").toString()));
                    ...
                    if ("A".equals(subProductInfo.get("ELEMENT_TYPE_CODE"))) {
                        ...
                        if (!"0".equals(actPlanId)) {
                            if ((resourcesInfo == null || resourcesInfo.size() == 0)
                                    && StringUtils.isNotEmpty(resourcesCode))
                            {
                                Map body = exchange.getIn().getBody(Map.class);
                                ...
                                try {
                                    lan.preData("ecaop.trades.qrychk.ParametersMapping", exchange);
                            ...
                            if (resourcesInfo != null && resourcesInfo.size() > 0) {
                                ...
                                tradeSubItem.add(LanUtils.createTradeSubItemG("deviceType",
                                        decodeTerminalType(terminalType), itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("deviceno", machineTypeCode, itemId));
                                tradeSubItem.add(LanUtils
                                        .createTradeSubItemG("devicebrand", orgdeviceBrandCode, itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("deviceintag", resBrandName, itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("mobilecost",
                                        String.valueOf(Integer.parseInt(cost) / 100), itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("mobileinfo", machineTypeName, itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("mobilesaleprice",
                                        String.valueOf(Integer.parseInt(salePrice) / 100), itemId));
                                tradeSubItem
                                        .add(LanUtils.createTradeSubItemG("resActivityper", resActivityper, itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("partActiveProduct",
                                        (String) msg.get("mainProduct"), itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("resourcesBrandCode", resBrandCode,
                                        itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("resourcesBrandName", resBrandName,
                                        itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("resourcesModelCode", resModelCode,
                                        itemId));
                                tradeSubItem.add(LanUtils
                                        .createTradeSubItemG("resourcesModelName", resModeName, itemId));

                                if (StringUtils.isNotEmpty(terminalSubtype)) // 有时为空
                                {
                                    tradeSubItem.add(LanUtils.createTradeSubItemG("terminalTSubType", terminalSubtype,
                                            itemId));
                                }
                                tradeSubItem.add(LanUtils.createTradeSubItemG("terminalType", terminalType, itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("isOwnerPhone", "0", itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("isPartActive", "0", itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("holdUnitType", "01", itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("resourcesType", "07", itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("packageType", "10", itemId));
                                tradeSubItem.add(LanUtils.createTradeSubItemG("itemid", itemId, itemId));
        ...
        if (productInfo != null && productInfo.size() > 0) {
            for (int i = 0; i < productInfo.size(); i++) {
                ...
                if ("1".equals(productMode)) {
                    ...
                    proparam.put("EPARCHY_CODE", "ZZZZ");
