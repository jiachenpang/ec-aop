...
public class HttpReq {
    ...
    public static synchronized MultiThreadedHttpConnectionManager multithreadsHttpConnectionManager() {
        if (mtHttpConnectionManager == null) {
            ...
            httpConnectionManagerParams.setDefaultMaxConnectionsPerHost(2); // 默认2
            httpConnectionManagerParams.setMaxTotalConnections(20); // 默认20
    ...
    protected HttpMethodBase createMethod() {
        ...
        if (reqMethod == HttpReqMethod.POST) {
            ...
            method.getParams().setParameter(HttpMethodParams.HTTP_CONTENT_CHARSET, contentCharset);
    ...
    private void shutdown(HttpMethodBase method) {
        ...
        if (!externalConnectionManager) ((SimpleHttpConnectionManager) httpConnectionManager).shutdown();
    }
    ...
    private HttpClient createHttpClient() {
        ...
        httpClientParams.setParameter(HttpMethodParams.RETRY_HANDLER, new DefaultHttpMethodRetryHandler(retryTimes, false));

        if (httpConnectionManager == null) httpConnectionManager = new SimpleHttpConnectionManager();
    ...
    private HttpResponse executeMethod(HttpClient httpClient, HttpMethodBase method) throws IOException {
        ...
        httpResponse.setResponseContent(method.getResponseBody());
    ...
    public HttpReq contentCharset(String contentCharset) {
        this.contentCharset = contentCharset;
        return this;
    }

    public static enum HttpReqMethod {GET, POST}
