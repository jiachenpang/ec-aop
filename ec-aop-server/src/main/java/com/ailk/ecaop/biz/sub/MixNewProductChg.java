 ...
 */
@EcRocTag("mixNewProductChg")
public class MixNewProductChg extends BaseAopProcessor implements ParamsAppliable {
    ...
    private static final String[] PARAM_ARRAY = { "ecaop.masb.chph.gifa.ParametersMapping",
            "ecaop.trade.cbss.checkUserParametersMapping", "ecaop.mxcg.preSub.ParametersMapping",
            "ecaop.trades.sccc.cancel.paramtersmapping", "ecaop.param.mapping.sfck"};
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        getThreePartInfo(exchange,body);
    ...
    public void checkOperatorMsg(Exchange exchange, Map body, Map msg) throws Exception {
        ...
        body.put("msg", msg);
        Exchange ex=ExchangeUtils.ofCopy(exchange, msg);
        ex.getIn().setBody(body);
        lan.preData(pmp[4], ex);
        CallEngine.aopCall(ex, "ecaop.comm.conf.url.esshttp.newsub");
        lan.xml2Json4ONS("ecaop.template.3g.staffcheck", ex);
        Map retStaffinfo = ex.getOut().getBody(Map.class);
    ...
    private Map preMixNumberInfo(Exchange exchange, Map msg) throws Exception {
        ...
        Map ext = new HashMap();
        String orderid = (String) GetSeqUtil.getSeqFromCb(pmp[0], ExchangeUtils.ofCopy(exchange, msg), "seq_trade_id",
                1).get(0);
        copymMsg.put("tradeId", orderid);
        ...
        msg.put("ext", ext);
        copymMsg.put("serviceClassCode", "00CP");
     ...
     * @throws Exception
     */
    private Map<String, Map<String, List>> preSingleProduct(Map msg, Map product, List<Map> threePartInfo)
            throws Exception {
        ...
        Map productInfo = n25Dao.qryProductInfo(MapUtils.asMap("COMMODITY_ID", productId, "PROVINCE_CODE", "00"
                + province));
        if (IsEmptyUtils.isEmpty(productInfo)) {
            throw new EcAopServerBizException("9999", "根据产品编码" + productId + "未查询到相应产品信息");
        }
        System.out.println("productInfo融合变更=====" + productInfo);
        ...
        for (Map element : packageElement) {
            ...
            temp.put("PACKAGE_ID", element.get("PACKAGE_ID"));
            temp.put("ELEMENT_ID", element.get("ELEMENT_ID"));
            ...
            // 判断该资费是否有关联的服务
            inMap.put("ELEMENT_ID", element.get("ELEMENT_ID").toString());
            List serviceIdList = n25Dao.selectServiceTraffic(inMap);
            System.out.println("走到44444444444444444");
            if (!IsEmptyUtils.isEmpty(serviceIdList)) {
                ...
                inMap.put("PACKAGE_ID", element.get("PACKAGE_ID"));
        ...
        System.out.println("走到555555555555555555555");
        ...
        System.out.println("走到666666666666666666666666");
        for (Map dis : discntList) {
        ...
        System.out.println("singleProduct融合变更=====" + singleProduct);
    ...
    private List<Map> preNoPackageElement(Map inMap, Object productId) {
        ...
        param.put("PRODUCT_MODE", inMap.get("PRODUCT_MODE"));
        param.put("EPARCHY_CODE", inMap.get("eparchyCode"));
        return n25Dao.qryDefaultPackageElementEcs(param);
    }
     ...
     * @throws Exception
     */
    private Map qryProductSingleElementInfo(Map inMap, String productStartDate, String productEndDate) throws Exception {// 将各个属性放在一个MAP，后期处理
        System.out.println("走到333333333333333333333333");
     ...
     * @throws Exception
     */
    private Map preDiscntItem(Map inMap, String productStartDate, String productEndDate) throws Exception {
        ...
        inMap.putAll(n25Dao.qryDiscntAttr(inMap));
    ...
    private Map preServiceItem(Map inMap, Object userId) throws Exception {
        System.out.println("走到7777777777777777777777777");
        List svcList = n25Dao.queryPackageElementInfo(inMap);
    ...
    private String dealProductEndDate(Map productInfo, Object optType, String startDate) throws Exception {
        System.out.println("走到222222222222222222222");
     ...
     * 三户信息查询
     */
    private void getThreePartInfo(Exchange exchange,Map body) {
        Map copymMsg = (Map) body.get("msg");
        ...
        copymMsg.put("custName", custInfo.get(0).get("custName"));// custName
    ...
    private Map preBaseData(Map msg, String appCode, Boolean isMixNum) {
        try {
            ...
            //subscribeId和tradeId全部改为从CB获取的tradeId 2017-3-20 by Zeng
    ...
    private Map preDataForTradeItem(Map msg, Boolean isMixNum) {
        try {
            ...
            if (isMixNum) {
                ...
                Item.add(LanUtils.createTradeItem("DEVELOP_STAFF_ID", msg.get("recomPersonId")));
